...
<%          
	...
	try {
		...
		ResultSet rs=stmt.executeQuery("select mem_name,mem_ssn,mem_pwd,mem_zip,mem_address,	mem_phone,mem_email,mem_job from member where mem_uid='"+ mem_uid +"'");
...
<form name=mform method=post action="update.jsp" >
	<table cellpadding=3 cellspacing=0 border=1 width=550 align=center>       
		...
		<tr>
			...
			<td bgcolor=white><input type=text name="name" size=20 maxlength=20 readonly value="<%=mem_name%>"></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td bgcolor=white colspan=2><input type=text name=zip size=7 maxlength=7 value="<%= mem_zip %>"></td>
		</tr>
		<tr>
			<td bgcolor="#eff4f8">&nbsp;주소<font color=red>&nbsp;*</font></td>
			<td bgcolor=white><input type=text name=addr size=50 maxlength=100 value="<%=mem_address%>"></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td bgcolor=white>
			  <select name=job class="formbox">
				...
				<option value="교원" <% if(mem_job.equals("자영업")){ %>selected<%}%>>교원
