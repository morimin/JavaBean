	...
	<div id=bbsview>
		<table border = 1>
			...
			<tr>
				...
				<td class="right"><a href="smhDownload.jsp?file=${file1 }"> ${file1 }</a></td>
				<!-- file1 을 클릭하면 하이퍼링크를 타고 다운로드  -->
			</tr>
			<tr>
				<td class="left"> 첨부파일2</td>
				<td class="right"><a href="smhDownload.jsp?file=${file2 }"> ${file2 }</a> </td>
			</tr>			
			<tr>
			...
			<tr>
				<td colspan = 2>
					<input type="button" value="목록" onClick="location.href='smhMain.jsp?CMDS=smhMakeList?bid=${param.bid}'" >
					<c:if test="${ sessionScope.sessLevel ge 9  }"> 
					<input type="button" value="수정" onClick="location.href='smhMain.jsp?CMDS=smhUpdateForm?bid=${param.bid}&idx=${param.idx}'" >
					...
					<input type="button" value="답글" onClick="location.href='smhMain.jsp?CMDS=smhReplyForm?idx=${param.idx}'" >					
					<input type="button" value="삭제" onClick="location.href='smhMain.jsp?CMDS=smhDeleteForm?bid=${param.bid}&idx=${param.idx}'">
					</c:if>
					<c:if test="${ sessionScope.sessLevel lt 9  }"> 
					<input type="button" value="수정" onClick="location.href='smhMain.jsp?CMDS=smhUpdateForm?idx=${param.idx}'" disabled>					
					<input type="button" value="답글" onClick="location.href='smhMain.jsp?CMDS=smhReplyForm?idx=${param.idx}'" disabled>					
