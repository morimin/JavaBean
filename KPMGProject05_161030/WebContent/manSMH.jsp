...
<% 
	...
	if ( sub.equals("1") )
	{
		...
		sql = "select max(ord) +1 as maxord from smhgroup";
		// ; 최고 큰값 +1 값으로 재정의
		...
		if (rs.next())
		{
			String tmp = Integer.toString(rs.getInt("maxord"));
			// ; 숫자를 문자로 바꿔서 tmp 재정의
			if ( tmp.equals("0"))
			{
				tmp = "1";  // default 로 1
			...
			request.setAttribute("neword", tmp);					
			
		}// if
		
		
		sql = String.format("select * from smhgroup order by ord asc");			
		pstmt = conn.prepareStatement(sql);
	...
	if ( sub.equals("2") )
	{
		String sql = "select * from smhgroup order by ord asc";  
		// ; 그룹정보를 일단 가져온다.
		pstmt = conn.prepareStatement(sql);
		rs = pstmt.executeQuery();
		...
		{  //  변경하기 위한 원본데이터를 수집
			...
			sql = "select * from smhmanager where idx='"+idx+"' ";
			...
			if ( rs.next())
			{
				...
				request.setAttribute("orgord", rs.getInt("ord"));
	...
	if ( sub.equals("3") )
	{
		String sql = "select *, smhgroup.title from smhmanager, smhgroup where smhmanager.grp = smhgroup.gidx order by grp asc, smhmanager.ord asc";  
		...
		{
			listTitle.add(rs.getString("title"));
			listGrpTitle.add(rs.getString("smhgroup.title"));  // ; for group title	
			listord.add(rs.getInt("ord"));			
			listUseflag.add(rs.getInt("Useflag"));
...
<table border=1>
	<tr height=30>
		<c:forEach var="i" begin="1" end="${menuListSize-1 }">
			...
			<c:if test="${sub eq i }">			
				<td width=150 align=center bgcolor="#ABCDEF"><a href="smhMain.jsp?CMDS=manSMH.jsp?sub=${i }">${menuList[i] } </td>
			</c:if>
			<c:if test="${sub ne i }">
				<td width=150 align=center bgcolor="#EEEEEE"><a href="smhMain.jsp?CMDS=manSMH.jsp?sub=${i }">${menuList[i] } </td>
			</c:if>
		...
		<table border=1>
			...
			<c:forEach var="i" begin="0" end="${bbsSize -1 }">
				<tr height=30>
					...
					<td>
						<input type ="button" value="수정" onClick="location.href='smhMain.jsp?CMDS=manSMH.jsp?sub=2&idx=${listIdx[i]}' ">
					</td> <!-- 비고 -->
				</tr>		
			</c:forEach>		
...
<c:if test="${sub eq 2 }">
	<c:if test="${!empty param.idx }"> <!-- idx 값이 있으므로 수정 -->
		...
		<c:set var="actionURL" value="smhUpdateManager" />
	</c:if>
	
	<c:if test="${empty param.idx }"> <!-- idx 값이 없으므로 등록 -->
		<c:set var="actionName" value="등록" />
		<c:set var="actionURL" value="smhInsertManager" />
	</c:if>
	...
	<form method=post action="${actionURL }">
		...
		<table border=1>
			...
			<tr height=30>
				<td>순서</td>
				<td><input type="text" name="ord" required value="${orgord }"></td>
				<td>사용</td>
				<td>
...
<c:if test="${sub eq 1 }">
	<form name =gForm method=post  action="smhInsertGroup">
	<!-- ; onSubmit="return checkError()" 대신 타이틀 입력 옆에 required 를 입력해도 된다. -->
		<table border=1>
			...
			<tr height = 30>
				<td> 순서 </td>
				<td> <input type ="text" name=ord value="${neword }" required></td>
				<td colspan=2>
	...
	<c:if test="${groupSize gt 0 }">
		<table border =1>
			...
			<c:forEach var="i" begin="0" end="${groupSize -1 }">
				<form method=post action="smhUpdateGroup">
				<input type=hidden name=gidx value="${listIdx[i] }">
					<tr height=30>
						<td> <input type="text" name="ord" value="${listord[i] }" style='width:30px;'> </td>
