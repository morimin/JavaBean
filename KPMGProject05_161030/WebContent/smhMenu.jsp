...
<% 
	...
	sql = String.format("select * from smhgroup where useflag='1' order by ord asc");
	...
	sql="select * from smhmanager where useflag='1' order by ord asc";
	pstmt = conn.prepareStatement(sql);
	rs = pstmt.executeQuery();
			...
			<c:if test="${bbsSize gt 0 }">
				<c:forEach var="j" begin="0" end="${bbsSize -1 }">
					<c:if test="${listGrp[j] eq listGidx[i] }" >
						<tr height=20>
							<td align="right" >
								<a href="smhMain.jsp?CMDS=smhMakeList?bid=${listIdx[j] }"> ${listTitle[j] }</a> 
							</td>
						</tr>					
					</c:if>				
				</c:forEach>			
