 ...
 */
@WebServlet("/smhMakeList")
public class SmhMakeList extends HttpServlet {
	String bid;
	String opt;
	String key;
	String page;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RequestDispatcher rd = request.getRequestDispatcher("smhMain.jsp?CMDS=smhList.jsp");
	...
	private SmhList readDB(int upperIdx, String page)  // 큰 수에서 10자리를 채우기 위함 for making List. 
	{										// 파라미터로 page 넣어줌
		
		SmhList list = new SmhList();
		...
		try{
			...
			sql = String.format("select * from smhmanager where idx='%s'", bid);
			...
			{ // 검색하겠다				
				...
				{
					sql = "select * from smh where bid='"+bid+"'  and title like '%" + key + "%' order by notice DESC, gid DESC,  thread ASC "; 
							
				}else if(opt.equals("2"))
				{
					sql = "select * from smh where bid='"+bid+"'  and writer like '%" + key + "%' order by notice DESC, gid DESC,  thread ASC ";
				}else
				{  // 이도저도 아니면 처음꺼 가져와서 보여줌
					sql = String.format("select * from smh where bid='%s' and idx < '%d'  order by notice DESC, gid DESC, thread ASC", bid, upperIdx);
				}
			...
			{ // 없다는 소리는 정상동작하겠다.
				...
				sql = String.format("select * from smh where bid='%s' order by notice DESC, gid DESC,  thread ASC limit %d, %d",bid, start, line);
				// ; 화면에 보여주는 쿼리가 이것임
			}												
			...
			// row카운트 계산
			sql = String.format("select count(*) as maxcount from smh where bid='%s'", bid);
			pstmt = conn.prepareStatement(sql);			
			rs = pstmt.executeQuery();
			rs.next();
