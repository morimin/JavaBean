 ...
 */
@WebServlet("/smhInput")
public class SmhInput extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if ( idx.equals("") )
			{
				// 새글 -> gid 받아오기
				//
				String gidSql = "select max(gid) + 1 as newgid from smh";  // bid가 어디가 됐든 관계없이 최대값 찾아오는 쿼리
				...
				// 답글 -> 원본글 찾아와야함.
				String orgSql = "select * from smh where idx='"+ idx+"'";  // 'idx'
				...
				if(rs.next()) // rs에 값이 존재하면
				{
					...
					//
					String threadSql = "select thread as result_thread, right(thread, 1) as result_right from smh where gid='"+gid+"' and length(thread) = length('"+orgThread+"') +1 and locate('"+orgThread+"', thread)=1 order by thread DESC limit 1 ";
			...
			String sql = String.format("insert into smh (title, writer, html, ymd, hms, file1, file2, head, notice, gid, thread, bid) values('%s', '%s', '%s', now(), now(), '%s', '%s', '%s', '%s','%d', '%s', '%s' )", title, writer, html, serverFileName1, serverFileName2, head, notice, gid, thread, bid );			
			...
			System.out.print("Input ERROR : " + e.getMessage() + "<br>");  
			// ; syso 이므로 콘솔에 출력
