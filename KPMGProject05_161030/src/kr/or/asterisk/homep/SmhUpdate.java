 ...
 */
@WebServlet("/smhUpdate")
public class SmhUpdate extends HttpServlet {
	String bid;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 첨부파일 처리 시작
			//
			String savePath = "/data/smh";  // 첨부파일에 대한 위치
			...
			String orgSql = String.format("select * from smh where idx='%s'", idx);
			...
			String sql = String.format("update smh set title='%s', writer='%s', html='%s', file1='%s', file2='%s', idx='%s', head='%s', notice='%s' where idx='%s' " , title, writer, html, serverFileName1, serverFileName2, idx, head, notice, idx);
			...
			{  // 그렇지 않으면 성공
				...
				request.setAttribute("nextURL", "smhMain.jsp?CMDS=smhView.jsp?idx="+idx+"&bid"+bid);
		...
		RequestDispatcher rd = request.getRequestDispatcher("smhMain.jsp?CMDS=smhResult.jsp?bid"+bid);  // 수정되면서 그 게시판으로 이동시키기 위함.
