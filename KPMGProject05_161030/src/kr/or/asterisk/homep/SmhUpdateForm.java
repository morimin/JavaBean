 ...
 */
@WebServlet("/smhUpdateForm")
public class SmhUpdateForm extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (idx == null)  // 만약 idx가 없으면 
		{
			...
			request.setAttribute("nextURL", "smhMain.jsp?CMDS=smhCover.jsp");
		...
		try{
			...
			String sql = String.format("select * from smh where idx = '%s' ", idx );			
			...
			{  // db가 있으면
				...
				request.setAttribute("ymd", rs.getString("ymd"));
				request.setAttribute("hms", rs.getString("hms"));
				request.setAttribute("gid", rs.getString("gid"));
				request.setAttribute("thread", rs.getString("thread"));
		...
		// if 문을 포워드 할것임
		RequestDispatcher rd = request.getRequestDispatcher("smhMain.jsp?CMDS=smhUpdateForm.jsp?idx="+idx);
		rd.forward(request, response);			
