 ...
 */
@WebServlet("/smhUpdateManager")
public class SmhUpdateManager extends HttpServlet {	
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			String sql = String.format("update smhmanager set title='%s', useflag='%s', ord='%s', head='%s', rlevel='%s', wlevel='%s', line='%s' where idx='%s' ", title, useflag, ord, head, rlevel, wlevel, line, idx );			
			...
			{  // 1보다 작으면 error
				...
				request.setAttribute("nextURL", "smhMain.jsp?CMDS=manSMH.jsp?sub=3");
