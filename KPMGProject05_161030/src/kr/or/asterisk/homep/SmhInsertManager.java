 ...
 */
@WebServlet("/smhInsertManager")
public class SmhInsertManager extends HttpServlet {	
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			String sql = String.format("insert into smhmanager (title, ord, useflag, grp, head, rlevel, wlevel, line)"
					+"values('%s','%s','%s','%s','%s','%s','%s','%s')", title, ord, useflag, grp, head, rlevel, wlevel, line);			
			...
			{  // 1보다 작으면 error
				...
				request.setAttribute("nextURL", "smhMain.jsp?CMDS=manSMH.jsp?sub=2");
